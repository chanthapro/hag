...
if(isset($_POST['btnsave'])){
	...
	if(isset($_FILES['pic1'])){
	  ...
	  $sql = "insert into tbl_activity (f_name,f_desc,c_id)values('".$txtname."','".$txtdesc."','".$txtSid."')";
...
if(isset($_GET['delete'])){
	...
	$photo = mysqli_query($conn,"select * from tbl_activity where f_id=".$id);
	$prow = mysqli_fetch_assoc($photo);
	unlink("upload/".$prow['f_img']);
    $query = mysqli_query($conn,"delete from tbl_activity where f_id=".$id);
	if($query){
		header("location:activity.php");
	}
}
...
if(isset($_GET['edit_id'])){
	$id = $_GET['id'] ?? null;
	$query = mysqli_query($conn,"select * from tbl_activity where f_id=".$id);
...
$sql = "update tbl_activity set f_name='".$txtname."', f_desc='".$txtdesc."',c_id='".$txtpid."' where f_id=".$id;
            ...
            <div class="col-sm-10 col-md-10 col-lg-10">
                    ...
                    <form role="form" method="post" action="<?php $_SERVER['PHP_SELF']; ?>"
                        ...
                        <div class="card-body">
                            <input type="hidden" class="form-control" id="txtid" name="txtid"
                                value="<?php echo $id; ?>">
                            <input id="id-img" type="hidden" class="form-control" id="txtimg" name="txtimg"
                ...
                ?>
                            <button type="reset" class="btn btn-danger"
                                onclick="window.location='activity.php'">Cancel</button>
...
<!--                                            <th>Image</th>-->
                                    ...
                                    <tbody>
                                        <?php
						$query = mysqli_query($conn,"select * from tbl_activity order by f_id  ");
						while($row = mysqli_fetch_assoc($query)){
						?>
                                        <tr>
